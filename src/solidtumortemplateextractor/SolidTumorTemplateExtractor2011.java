...
public class SolidTumorTemplateExtractor2011 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Unexpected Events Processed:\n\n\n");

              System.out.println(EventProcessor.process(output.toString()));
    }
    ...
    public static String getTimepointData(Sheet sheet, int start, String headers) {
        ...
        sheetInfo.append("\t").append(getDateValue(row.getCell(2)));  // this is the cut off value for both sheets and timepoints
        ...
        for (int i = 4; i < 24; i = i + 2) {
            ...
            if (!mouse.isBlank() && !x.isBlank() && !y.isBlank()) {
                ...
                // tumor, subtype,passage
                
                // for uthscsa
                //tumor = mouse.split("p")[0].trim();
                
                if(tumor.equals("NCH-S13-7484")) tumor = "NCH-S13_7484";
                
                if(tumor.equals("ES4")) tumor = "ES-4";
                if(tumor.equals("ES7")) tumor = "ES-7";
                
                if(tumor.equals("TC71")) tumor = "TC-71";
                if(tumor.equals("T C71")) tumor = "TC-71";
                
                // for mda 2011
                if(tumor.startsWith("OS")){
                    tumor = tumor.replace("-","");
                }
                
                timepoints.append(tumor).append("\t");
                timepoints.append(IDChecker.getSubtype(tumor)).append("\t");
    ...
    private static String getHeaders(Sheet sheet) {
        ...
        passage = getValue(sheet.getRow(4).getCell(2));
    ...
    private static String getTreatment(Sheet sheet) {
        ...
        String comment = getValue(sheet.getRow(7).getCell(2));
            ...
            // specific to 1817_2 MDA
        return comment+" "+treatmentAndDose;// + " " + comment;
