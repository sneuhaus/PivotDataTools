 ...
 * @author sbn
 */
public class ALLTemplateExtractor1819 {
    ...
    static final String BEFORE_DATE = "01/01/2023";
    static final String AFTER_DATE = "01/01/2010";
    ...
    private static String getValue(Cell cell, CellType type) {
     ...
     private static String getUSDateValue(Cell cell) {
         
          SimpleDateFormat usSdf = new SimpleDateFormat();
        usSdf.applyPattern("MM/dd/yyyy");
        ...
        try {
            ...
            return usSdf.format(cell.getDateCellValue());
        } catch (Exception e) {
        }
        try {
            Date d = new Date(getValue(cell));
            return usSdf.format(d);
        } catch (Exception e) {
        }
    ...
    //    String dayZero = getValue(sheet.getRow(5).getCell(7));
        ...
        int rowCount = 1;
            ...
            if (!NULL_CELL.equals(armTesting) && !armTesting.isBlank()) {
                String arm = "A";//getValue(sheet.getRow(row).getCell(0));
        ...
        //        String schedule2 = getValue(sheet.getRow(row + 3).getCell(1));
                ...
                for (int mouse = 9; mouse < 40; mouse++) {
                    ...
                    String dayZero = getDateValue(sheet.getRow(row + mouse).getCell(3));
                    String mouseName = subtype +" "+getValue(sheet.getRow(row + mouse).getCell(4))+" "+getValue(sheet.getRow(row + mouse).getCell(5));
                    String codes = getValue(sheet.getRow(row + mouse).getCell(lastDataCol));
                    String footNote = getValue(sheet.getRow(row + mouse).getCell(lastDataCol+1));
                    String deathDate = getUSDateValue(sheet.getRow(row + mouse).getCell(lastDataCol+2));

                    if (mouseName != null && !mouseName.isBlank()) {
                        loop:
                        for (int i = 6; i < lastDataCol; i++) {
                            ...
                            try {
                                ...
                                String inoculationDate = "08/25/2021";
                                if(mouseName.contains("ALL-85")){
                                    inoculationDate = "09/01/2021";
                                }
